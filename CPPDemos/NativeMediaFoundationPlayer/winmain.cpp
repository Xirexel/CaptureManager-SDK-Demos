...
VOID WINAPI CheckRendererItem(HWND hwnd, UINT uID)
{
	...
	CheckMenuRadioItem(
		...
		ID_WINDOW_HANDLER,           // first item in range 
		ID_DX9_SWAPCHAIN,          // last item in range 
		uID,                // item to check 
		MF_BYCOMMAND        // IDs, not positions 
		);

	g_pPlayer->setRenderType(uID);
}

VOID WINAPI CheckLibraryItem(HWND hwnd, UINT uID)
{
	HMENU hmenuBar = GetMenu(hwnd);
	...
	CheckMenuRadioItem(
		hmenuBar,         // handle to menu 
		ID_LIB_CM,           // first item in range 
		ID_LIB_VR,          // last item in range 
		uID,                // item to check 
		MF_BYCOMMAND        // IDs, not positions 
		);

	g_pPlayer->setLibraryType(uID);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		default:
			...
			if ((LOWORD(wParam)&0x0F00) == 0x0200)
			{
				CheckRendererItem(hwnd, LOWORD(wParam));

				break;
			}
			else if ((LOWORD(wParam) & 0x0F00) == 0x0300)
			{
				CheckLibraryItem(hwnd, LOWORD(wParam));
...
LRESULT OnCreateWindow(HWND hwnd)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		CheckRendererItem(hwnd, ID_WINDOW_HANDLER);

		CheckLibraryItem(hwnd, ID_LIB_CM);
...
void UpdateUI(HWND hwnd, PlayerState state)
{
	...
	case Started:
		bPlayback = TRUE;
		::KillTimer(hwnd,  TIMERID_REFRESH);
		::SetTimer(hwnd, TIMERID_REFRESH, FREQUENCY_REFRESH, TimerIDRefresh);
		break;
