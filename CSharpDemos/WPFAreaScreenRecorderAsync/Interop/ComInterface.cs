...
namespace WPFAreaScreenRecorderAsync.Interop
{
    ...
    internal static class ComInterface
    {
        ...
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate int CreateDevice_Ex(IDirect3D9Ex d3D9, uint Adapter, int DeviceType, IntPtr hFocusWindow, int BehaviorFlags, NativeStructs.D3DPRESENT_PARAMETERS pPresentationParameters, out IDirect3DDevice9 ppReturnedDeviceInterface);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate int CreateDevice(IDirect3D9 d3D9, uint Adapter, int DeviceType, IntPtr hFocusWindow, int BehaviorFlags, NativeStructs.D3DPRESENT_PARAMETERS pPresentationParameters, out IDirect3DDevice9 ppReturnedDeviceInterface);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate int CreateRenderTargetEx(IDirect3DDevice9Ex device, uint Width, uint Height, int Format, int MultiSample, uint MultisampleQuality, int Lockable, out IDirect3DSurface9 ppSurface, ref IntPtr pSharedHandle);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate int CreateRenderTarget(IDirect3DDevice9 device, uint Width, uint Height, int Format, int MultiSample, uint MultisampleQuality, int Lockable, out IDirect3DSurface9 ppSurface, ref IntPtr pSharedHandle);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate int CreateTextureEx(IDirect3DDevice9Ex device, uint Width, uint Height, uint Levels, uint Usage, int Format, int Pool, out IDirect3DTexture9 ppTexture, ref IntPtr pSharedHandle);

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate int CreateTexture(IDirect3DDevice9 device, uint Width, uint Height, uint Levels, uint Usage, int Format, int Pool, out IDirect3DTexture9 ppTexture, ref IntPtr pSharedHandle);
